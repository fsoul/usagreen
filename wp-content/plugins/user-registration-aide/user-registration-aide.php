		...
		if( $user ){//|| $options['lockout_invalid_usernames'] == "yes" ) {
			if( $user === false ){ 
				...
				if( $approve == 1 ){
					...
					if( $status == 'pending' ){
						$error = sprintf( __( '<p align="center"><strong>ERROR</strong>: We are sorry but your user account must first be approved by an Administrator!<br /><br />Please contact an Administrator or try again later.<br/><a href="%s">Home</a></p>', 'user-registration-aide' ), site_url() );
						//exit( $error );
					}elseif( $status == 'denied' ){
						$error = sprintf( __( '<p align="center"><strong>ERROR</strong>: We are sorry but your user account has been denied by an Administrator!<br /><br />Please contact an Administrator if you feel this is an error.<br/><a href="%s">Home</a></p>', 'user-registration-aide' ), site_url() ) ;
						exit( $error );
