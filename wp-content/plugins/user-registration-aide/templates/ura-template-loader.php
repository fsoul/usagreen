...
function ura_get_template_part( $slug, $name = '' ){
	...
	//exit( $name );
	if( $name == 'reset-password' || $name == 'change-password' || $name == 'update-password' ){
	...
	if ( !locate_template( array( $filename, URA_TEMPLATE_URL.'/'.$filename ), true, false ) ) {
		...
		wp_enqueue_style( 'templates_style', plugins_url( 'css/templates-style.css', __FILE__ ) );
		wp_enqueue_style( 'parent_style',  $style );
...
function ura_body_classes_check(){
	if ( has_filter( 'body_class', 'twentyfourteen_body_classes' ) ) {
