...
class URA_REGISTRATION_FORM_VIEW
{
	...
	*/
	...
	function display_password_fields(){
		...
		$fieldName2 = ( string ) 'Confirm Password';
		...
		<p class="user-pass1-wrap">
			...
			<div class="wp-pwd">
				<span class="password-input-wrapper">
					...
					<div id="pass-strength-result" class="hide-if-no-js" size="20" aria-live="polite">
					<?php _e( 'Strength Indicator', 'user-registration-aide' ); ?>
					</div>
				</span>
			</div>
		</p>
		<label for="xwrd2"><?php _e( 'Confirm Password' ) ?></label><br />
		<input type="password" name="xwrd2" id="xwrd2" class="input" size="20" value="" autocomplete="off" />
		<br class="clear" />
		<?php
		/*
	...
	*/
	...
	function display_other_fields_theme_my_login( $fieldKey, $fieldName, $value ){
		if( $fieldKey != 'user_pass' ){
			...
			?>
			<p>
			<label><?php _e( $label, 'user-registration-aide' ); ?><br />
	...
	*/
	...
	function display_known_fields( $fieldKey, $fieldName, $value ){
		$label = apply_filters( 'create_kf_label', $fieldKey, $fieldName );
		?>
		<div class="large-3 medium-12 columns"><label>
		<?php
		if( $fieldKey == 'first_name' ){
			...
			?>
			<?php _e( 'Nickname', 'user-registration-aide' ); ?><?php echo $label; ?>
			<?php
		}elseif( $fieldKey == 'user_url' ){
			?>
			<?php _e( 'Website', 'user-registration-aide' ); ?><?php echo $label; ?>
			<?php	
		}else{
			?>
			<?php _e( $fieldName, 'user-registration-aide' ); ?><?php echo $label; ?>
			<?php
		}
		?>
		<!-- test 14 -->
		...
		<input type="text" name="<?php echo $fieldKey; ?>" id="<?php echo $fieldKey; ?>" autocomplete="on" class="input" size="25" pattern="[A-z-А-я]+" required/></label>
		</div>
		<!-- first/last name -->
		<?php
	}
	...
	*/
	...
	function display_known_fields_theme_my_login( $fieldKey, $fieldName, $value ){
		...
		if( $fieldKey == 'first_name' ){
			?>
			<label><?php _e( 'First Name'.$label, 'user-registration-aide' ); ?><br />
			<?php
		}elseif( $fieldKey == 'last_name' ){
			?>
			<label><?php _e( 'Last Name'.$label, 'user-registration-aide' ); ?><br />
			<?php
		}elseif( $fieldKey == 'nickname' ){
			?>
			<label><?php _e( 'Nickname'.$label, 'user-registration-aide' ); ?><br />
			<?php
		}elseif( $fieldKey == 'user_url' ){
			?>
			<label><?php _e( 'Website'.$label, 'user-registration-aide' ); ?><br />
			<?php	
		}else{
			?>
			<label><?php _e( $fieldName.$label, 'user-registration-aide' ); ?><br />
			<?php
		}
		?>
