 ...
 */
?>
<div class="fw-table fw-service-table">
	<table>
				...
				<tr class="<?php echo esc_attr($row['name']); ?>">
					<?php foreach ( $atts['table']['cols'] as $col_key => $col ) : ?>
						<td class="<?php echo esc_attr($col['name']); ?>">
							<?php 
								if (($atts['table']['content'][ $row_key ][ $col_key ]['textarea'] == 'yes') || ($atts['table']['content'][ $row_key ][ $col_key ]['textarea'] == 'no')) {
									echo "<div class=\"service-{$atts['table']['content'][ $row_key ][ $col_key ]['textarea']}\"></div>"; 
								} else { ?>
								<?php echo $atts['table']['content'][ $row_key ][ $col_key ]['textarea']; ?>
							<?php	} ?>
						</td>
